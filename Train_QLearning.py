...
def Qtrain(id, global_model, global_optimizer, args, eps_start=1.0, epsilon_decay = 100000, Itarget=5, IAsyncUpdate=100):
    ...
    if gen <= 0.4:
        ...
        eps_final = 0.5
    #print(gen)   #TODO two choose 0.5 may be bad
    for episode in range(args.num_episodes):
        ...
        while True:
            ...
            qvals = torch.unsqueeze(qvals[0][action],0)
            ...
            next_qvals_max = torch.max(next_qvals, dim=-1)[0]
            target_qvals = reward + (1 - done) * args.gamma * next_qvals_max.detach()
